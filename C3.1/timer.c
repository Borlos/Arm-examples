typedef volatile struct timer{
    unsigned int *base;         //timer's base address
...
void timer_init(){
    ...
    for (i=0; i<4; i++){
        tp = &timer[i];
        if (i==0) tp->base = (unsigned int *)0x101E2000;
        if (i==1) tp->base = (unsigned int *)0x101E2020;
        if (i==2) tp->base = (unsigned int *)0x101E3000;
        if (i==3) tp->base = (unsigned int *)0x101E3020;
        ...
        *(tp->base+TBGLOAD) = 0x1C00; // timer counter value
        ...
        strcpy((char *)tp->clock, "00:00:00"); //need to implement *memcpy in string.c!
    }
}
...
void timer_handler(int n) {
    ...
    for (i=0; i<8; i++){ //Clear chars
        unkpchar(t->clock[i], n, 70+i);
    }
    if (t->tick==120){ //Update time
        t->tick = 0; 
        t->ss++;
        if (t->ss == 60){
            t->ss = 0; 
